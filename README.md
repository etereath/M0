...
当前正式版本：`v0.2.1`；其 Release ZIP 与 `v0.2.0` 契约语义一致；历史冻结基线：`v0.1.0`。
...
M0 的 `v0.1.0` 业务语义已冻结，当前正式契约版本为 `v0.2.1`。`v0.2.1` Release ZIP 与 `v0.2.0` 的契约语义一致，仅修正发布文档和版本元数据。所有 M1–M7 项目必须统一固定依赖 `v0.2.1` Release、Tag 或 commit，不得依赖 `main`，也不得复制后在本地修改契约。所有后续变更必须先通过 Issue，再通过 Pull Request。
